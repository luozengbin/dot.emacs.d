 ...
 */
 ...
 /* fix newline bug
  * http://highmt.wordpress.com/2010/08/23/cygwin-1-7-6-lgrep-4-51-on-ntemacs-23-2/ 
  */
    ...
    for( f->eof = FALSE ; FALSE == f->eof ; ){
        ...
        if( TRUE == flagMatched || FALSE == grep_mode ){
            if( TRUE == simple ){
                for( i = 0 ; i < len ; i++ )
                    putchar( str[ i ] );
                /* fix grep result append newline */
                if( str[len - 1] != newline_cd ){
                    putchar(newline_cd);
                }
            } else {
                if( len > (STR_SIZE >> 2) ){
                    ...
                    /* fix grep result append newline */
                    if( encoding_space[len - 1] != newline_cd ){
                        putchar(newline_cd);
                    }
                    free( encoding_space );
                } else {
                    ...
                    /* fix grep result append newline */
                    if( encode_str[len - 1] != newline_cd ){
                        putchar(newline_cd);
                    }      
                }
            }
